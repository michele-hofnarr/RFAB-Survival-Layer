...
Разработано и тестировалось для **RFAB SE XI • Kazan Edition [ver. 25.09.2026]**.
...
| **Тканевый стресс** | чистая льняная ткань действует как лекарство. Пока болезнь ещё не началась, счётчик сам откатывается вне боя |
...
| **Спальный мешок** | перк «Основы выживания»; шьётся на дубильном станке из 4 кож. Разместить — выложить из инвентаря на землю; забрать — зажать кнопку действия (по умолчанию `E`) |
...
Мод приходит включённым: виджет появляется на экране сразу, а в разделе
«Помощь» — строка **«[RFAB] Режим выживания»**.

Выключатель **«Мод включён»** лежит в «Настройках модов» → **RFAB Survival** →
«Общее». Снятая галочка останавливает все правила и снимает с персонажа всё,
что мод успел применить; виджет при этом пропадает.
...
болезни, предметы и заклинания, в том числе из избранного. Поэтому отдельное
«чистое» сохранение без мода не нужно — достаточно один раз загрузиться с
выключенным модом и пересохраниться.

1. **Закройте игру.**
2. **Удалите старую версию** мода в MO2.
3. **Установите новую.**
4. Запустите игру и загрузите сохранение.
5. Откройте «Настройки модов» → **RFAB Survival** → «Общее» и **снимите
   галочку «Мод включён»**.
6. **Сохранитесь** и загрузите это сохранение.
7. Поставьте «Мод включён» обратно и играйте дальше.

Настройки при этом не теряются: они лежат в
`MCM/Settings/RFAB_SurvivalLayer.ini`, отдельно от сохранения. Вернуть заводские
значения можно кнопкой на странице «Общее» — она возвращает и «Мод включён», то
есть сброс настроек мод не выключает.
