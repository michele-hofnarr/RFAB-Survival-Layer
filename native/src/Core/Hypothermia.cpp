...
namespace RSL
{
    ...
    void Hypothermia::OnStageChanged(std::int32_t a_old, std::int32_t a_stage)
    {
        if (a_stage >= 3 && a_old < 3) {
            ...
            // talking: the ability was not instantiating at all. With the
            // record fixed there is nothing to work around.
            SetLock(true, false);
        } else if (a_stage < 3 && a_old >= 3) {
            Diseases().Get(Id()).stage3Seconds = 0.0f;
            SetLock(false, true);
        }
    ...
    void Hypothermia::SyncRestBlock()
    {
        ...
        // pass so it follows the player in and out of cover.
        bool wanted = false;
        if (Stage() >= 1) {
